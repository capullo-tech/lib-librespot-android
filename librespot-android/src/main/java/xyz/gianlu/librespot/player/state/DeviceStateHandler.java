...
public final class DeviceStateHandler implements Closeable, DealerClient.MessageListener, DealerClient.RequestListener {
    ...
    public synchronized void updateState(@NotNull Connect.PutStateReason reason, int playerTime, @NotNull Player.PlayerState state) {
        // Jetispot patch: tolerate null connectionId. The dealer's hm://pusher/v1/connections/
        // message can be dropped if it arrives before DeviceStateHandler subscribes, in which
        // case connectionId stays null forever. Upstream throws IllegalStateException here and
        // kills the player thread on the first state push. We silently skip the put-state
        // update — local playback works fine without Spotify Connect device registration.
        if (connectionId == null) {
            LOGGER.debug("updateState skipped: not registered with Spotify Connect (dropped connection_id race)");
            return;
        }
